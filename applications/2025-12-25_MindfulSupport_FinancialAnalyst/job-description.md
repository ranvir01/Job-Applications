# 📋 Job Description: Mindful Support Services Financial Analyst

---

## 🏢 Company Information

| Field | Details |
|-------|---------|
| **Company** | Mindful Support Services (Mindful Therapy Group) |
| **Role** | Financial Analyst |
| **Location** | Mountlake Terrace, WA (On-site) |
| **Job Type** | Full-Time |
| **Posted Date** | December 2025 |
| **Application Deadline** | Not specified |
| **Job Link** | *[Add link when applying]* |
| **Salary Range** | $85,000 - $100,000 per year |

---

## 📝 Job Description

```
About the Company:
We are a mental health organization focused on business-to-business support services for independent therapy and psychiatric private practices. We provide administrative and organizational services to simplify the processes of lead-generation, marketing, billing, and collecting payments from patients and insurers. Our teams support over 2,300 mental health providers in 19 locations and via tele-health across 6 states.

We have built the Mindful Therapy Group brand from the ground up with over 14 years of dedication to solving the complex processes of the healthcare landscape in innovative ways.

Role Overview:
Mindful Support Services is actively seeking a proactive, detail-oriented Financial Analyst who thrives in a professional, fast-paced environment. As a key Finance employee, the Financial Analyst will be responsible for financial reporting, forecasting and budgeting, and other financial modeling. This is a great position for someone who enjoys analyzing business performance to support senior management in making meaningful tactical and strategic decisions.

Responsibilities:

Financial Reporting & Close:
- Prepare and review monthly, quarterly, and annual financial reports in accordance with GAAP and internal policies
- Lead monthly revenue calculation and analysis process working cross-functionally with accounting and payments teams
- Partner with accounting to ensure data integrity, consistency, and accuracy across financial statements
- Provide superior and timely customer service in responding to internal and external financial and payments-related inquiries
- Support the monthly close process, including journal entries, account reconciliations, variance analysis, and management reporting
- Contribute to the development and improvement of financial reporting processes, controls, and documentation

Financial Analysis & Decision Support:
- Perform detailed variance analysis versus budget, forecast, and prior periods, identifying key drivers and trends
- Develop and maintain financial models to support operational and strategic decision-making
- Provide actionable insights to finance leadership and cross-functional partners through clear, concise analysis and storytelling
- Support evaluation of new initiatives, investments, and growth opportunities

Budgeting & Forecasting:
- Play a key role in the annual budgeting and periodic forecasting processes
- Collaborate with department leaders to understand assumptions, drivers, and risks
- Analyze forecast accuracy and continuously refine planning models and methodologies

Ad Hoc & Special Projects:
- Lead and support ad hoc analytical projects related to growth, efficiency, profitability, and operational performance
- Assist with system implementations, reporting enhancements, and data automation initiatives
```

---

## ✅ Requirements

### Required Qualifications
- Bachelor's degree with a focus in finance, accounting, economics or related field
- 2-5+ years of financial planning and analysis experience including financial reporting and budgeting experience
- Excellent analytical, decision-making, and problem-solving skills
- Self-starter with ability to set and balance priorities, work independently, and take ownership
- Attention to accuracy and detail
- Great interpersonal and communication skills, both oral and written
- Intermediate to Advanced knowledge of Microsoft Office Suite, especially Excel and PowerPoint

### Preferred Qualifications
- MBA or advanced degree
- Experience with ERP systems (e.g. Sage, NetSuite)
- Experience with data visualization (e.g. Power BI, Tableau)
- Experience with medical or mental health industry

---

## 🔑 Keywords & Skills Mentioned

| Category | Keywords |
|----------|----------|
| **FP&A Core** | Budgeting, forecasting, variance analysis, financial modeling, financial reporting |
| **Close Process** | Monthly close, journal entries, account reconciliations, GAAP, management reporting |
| **Analysis** | Trend analysis, key drivers, actionable insights, storytelling with data |
| **Cross-Functional** | Accounting partnership, department collaboration, payments teams |
| **Technical** | Excel, PowerPoint, ERP (Sage, NetSuite), Tableau, Power BI |
| **Industry** | Healthcare, mental health, medical billing |

---

## 🎯 Match Analysis

### My Matching Qualifications
- ✅ **Bachelor's in Finance** – BA Business Administration with Finance concentration from UW Foster (3.6 GPA)
- ✅ **2-5+ years FP&A experience** – 4+ years across consulting and business management
- ✅ **Financial reporting & budgeting** – Monthly P&L, $500K+ budget management, executive reporting
- ✅ **Variance analysis** – Core skill in both current and previous roles
- ✅ **Month-end close support** – Partnered with accounting on close, calculated accruals
- ✅ **Financial modeling** – DCF, valuations, forecasting models
- ✅ **Excel (Advanced)** – VBA, Pivot Tables, complex formulas; automated reporting by 80%
- ✅ **Data visualization** – Tableau experience
- ✅ **Cross-functional collaboration** – Worked with accounting, operations, leadership
- ✅ **Storytelling with data** – Executive presentations, Board packages
- ✅ **Self-starter / ownership mentality** – Independent contractor experience, managed entire finance function

### Gaps to Address
- ⚠️ **No healthcare/mental health experience** – Frame as: "Transferable skills from service-based businesses; eager to apply to healthcare"
- ⚠️ **No Sage/NetSuite experience** – Emphasize: "Experience with Salesforce and enterprise systems; quick to learn new platforms"

---

## 💡 Application Strategy

### Resume Focus Areas
- **Full FP&A cycle** – Budgeting, forecasting, variance analysis, close support, reporting
- **Month-end close involvement** – Accruals, reconciliations, accounting partnership
- **Financial modeling** – Decision support, scenario analysis
- **Process improvement** – Automation, efficiency gains (80% time savings)
- **Cross-functional** – Collaboration with accounting, operations, leadership
- **Data storytelling** – Clear communication, executive presentations

### Key Selling Points
1. **Local** – Mountlake Terrace is ~20 min from Seattle; can be on-site easily
2. **Full FP&A experience** – 4+ years covering exactly what they need
3. **Automation track record** – "Reporting enhancements and data automation" directly matches
4. **Close support experience** – Accruals, reconciliations, variance analysis
5. **Ownership mentality** – "Self-starter" matches consulting/entrepreneurial background

### Why This Role is Appealing
- **Location**: On-site in Mountlake Terrace—very close to your area
- **Salary**: $85K-$100K is solid for the Seattle area
- **Mission**: Mental health access is meaningful work
- **Growth**: Supporting 2,300+ providers across 6 states = scale
- **Culture fit**: "Can-do attitude," "ownership," "adaptable" match your profile

---

## 🔗 Research Notes

### About the Company
- **Industry**: Mental health support services (B2B for therapy practices)
- **Services**: Billing, payments, lead generation, marketing for independent therapists
- **Scale**: 2,300+ providers, 19 locations, 6 states, plus telehealth
- **Brand**: Mindful Therapy Group
- **History**: 14+ years, built from ground up
- **Mission**: Improved access to high-quality mental healthcare

### Company Culture Traits (from JD)
- Strategic Thinker
- Positive Energy
- Direct Communicator
- Adaptable
- Can-Do Attitude
- Team Player

### Benefits
- 75% employer-covered Health, Dental & Vision
- 401(k) with employer matching
- 8 paid holidays
- 15 PTO days (first year)
- Professional development opportunities

---

*Date Saved: 2025-12-25*

